...
import { getAtletaByBid, getClubeById, getConquistasByBid } from "@/lib/mock-data";
...
export default function DossiePage() {
  ...
  const a = getAtletaByBid(Number(bid));

  if (!a) {
    return (
      <div className="border border-border bg-surface p-10 text-center">
        <p className="text-muted">{t("dossie.notFound")}</p>
        <Button href="/atletas" variant="secondary" className="mt-4">
  ...
  const conquistas = getConquistasByBid(a.bid);
      ...
      <section className="border border-border bg-surface p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            <ClubeCrest src={club?.webpCrestUrl ?? null} name={club?.name ?? "?"} size={56} />
            <div>
              <h1 className="text-2xl font-extrabold tracking-tight">{a.name}</h1>
              <p className="text-sm text-muted">
                {a.apelido ? `"${a.apelido}" · ` : ""}BID {a.bid} · {club?.name} · {a.currentCategory}
              </p>
              <div className="mt-2 flex flex-wrap gap-2">
                <Badge tone="brand">{a.mainPosition}</Badge>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 border-t border-border pt-6 sm:grid-cols-4">
          <Info label={t("dossie.age")} value={`${a.age} ${t("hero.years")}`} />
          <Info label={t("dossie.birth")} value={formatDate(a.birthDate)} />
          <Info label={t("dossie.height")} value={`${a.heightCm} cm`} />
          <Info label={t("dossie.weight")} value={`${a.weightKg} kg`} />
          <Info label={t("dossie.foot")} value={t(`foot.${a.dominantFoot}` as TranslationKey)} />
        ...
        <div className="space-y-4">
          {isClaimed ? (
            <Panel title={t("dossie.agent.title")}>
              <AgentContact agentId={a.agentId} youtubeUrl={a.youtubeVideoUrl} />
            </Panel>
          ) : (
                  ...
                  <li key={c.id} className="flex items-center gap-3 text-sm">
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block text-xs text-muted">
                        {t(`dossie.conquistas.${c.tipo}` as TranslationKey)} · {c.ano}
                      </span>
                    </span>
                  </li>
                ))}
