...
import { loadAtletaDossie, loadConquistas, loadAgenteContact, loadEvolucaoReal } from "@/lib/services/atletas";
...
export default async function DossiePage({ params }: { params: Promise<{ bid: string }> }) {
  ...
  const [conquistas, evolucao, agent] = await Promise.all([
  ...
  return <AtletaDossie atleta={atleta} conquistas={conquistas} agent={agent} evolucao={evolucao} />;
}
