...
test("resolves both teams' full display name + FERJ numeric club id from the match page HTML", () => {
  ...
  assert.deepEqual(teams.home, { name: "E.C Nova Cidade", clubId: 64, crestUrl: "https://static.fferj.com.br/media/upload/time/img_icone_64.gif" });
  assert.deepEqual(teams.away, { name: "Futebol Clube Rio de Janeiro", clubId: 916, crestUrl: "https://static.fferj.com.br/media/upload/time/img_icone_929.jpg" });
});
...
test("composed match carries tournament/score/club identity from both sources", () => {
  ...
  assert.deepEqual(sumula.match.home, { name: "E.C Nova Cidade", clubId: 64, crestUrl: "https://static.fferj.com.br/media/upload/time/img_icone_64.gif" });
  assert.deepEqual(sumula.match.away, { name: "Futebol Clube Rio de Janeiro", clubId: 916, crestUrl: "https://static.fferj.com.br/media/upload/time/img_icone_929.jpg" });
...
test("captures the card reason (Session 55) — the HTML event's own motivo sentence, split from the ALL-CAPS player name", () => {
  const amaury = sumula.match.appearances.find((a) => a.bira === "243929");
  const matheus = sumula.match.appearances.find((a) => a.bira === "243429");
  assert.deepEqual(amaury.yellowCardReasons, ["Dar uma entrada contra um adversário de maneira temerária"]);
  assert.deepEqual(matheus.yellowCardReasons, ["Reclamação á arbitragem com palavras"]);
  assert.equal(amaury.redCardReasons, undefined);
});
