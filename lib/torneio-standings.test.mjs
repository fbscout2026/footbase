...
test("computeTopScorers: sums goals per athlete across appearances, sorted descending", () => {
  const appearances = [
    { bid: 1, name: "Zico", goals: 2 },
    { bid: 2, name: "Pele", goals: 1 },
    { bid: 1, name: "Zico", goals: 1 }, // same athlete, another match
    { bid: 3, name: "Ronaldo", goals: 0 }, // no goals — excluded
  ];
  const scorers = computeTopScorers(appearances);
  assert.deepEqual(scorers, [
    { bid: 1, name: "Zico", goals: 3 },
    { bid: 2, name: "Pele", goals: 1 },
  ]);
});
