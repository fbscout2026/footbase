...
export function buildFerjAppearances(
  ...
  for (const side of ["home", "away"] as const) {
    ...
    for (const p of players) {
      ...
      // "Entrou" row for them in the súmula's own Substituições table — a real gap in
      // the source's own data entry (see parse-cbf-events.ts's identical fix, Session
      // 57, for the confirmed real FES/FGF cases this pattern was found in).
      const scorerGoals = goals.filter((g) => !g.ownGoal && g.side === side && g.bira === p.bira);
