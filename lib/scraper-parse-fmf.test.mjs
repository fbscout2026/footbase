...
test("recovers the full name via the capital-then-lowercase transition when the apelido is not a literal prefix (e.g. mid-name nickname 'Portugal')", () => {
  // Session 57: upgraded from a repeat-only heuristic (which really did give up
  // here, matching the old assertion below) to the same full 3-pattern algorithm
  // CBF/FERJ use — "Portugal" + "Tiago Portugal Moraes" has no literal prefix
  // repeat, but does have a genuine capital-then-lowercase split ("l|Tiago"),
  // which pattern 2 now recovers correctly.
  const tiago = match.athletes.find((a) => a.bid === 927276);
  assert.equal(tiago.name, "Tiago Portugal Moraes");
});
