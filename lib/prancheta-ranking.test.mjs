...
function athlete(fbId, position, overrides = {}) {
  return {
    fbId,
...
test("montagem não repete atletas e deixa vagas sem candidato compatível", () => {
  ...
  assert.equal(new Set(lineup.map((entry) => entry.fbId)).size, 3);
});
...
test("posição principal vence secundária com desempenho equivalente", () => {
  ...
  assert.equal(ranked[0].candidate.fbId, 1);
...
test("desempenho superior lidera e disciplina aplica penalidade", () => {
  ...
  assert.equal(ranked[0].candidate.fbId, 1);
  assert.ok(ranked.find((entry) => entry.candidate.fbId === 3).score < ranked[0].score);
});
...
test("banco exclui titulares e ordena por nota", () => {
  ...
  assert.deepEqual(bench.map((entry) => entry.fbId), [3, 2]);
});
