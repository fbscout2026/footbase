...
// Session 55 incident: the FGF súmula carries no federation marker of its own
// (it's literally CBF's own PDF template), and the FGF adapter used to call
// this parser without setting `federation` — so every FGF tournament was
// silently recorded as `federation: "CBF"` in production, folding Rio Grande
// do Sul state tournaments into the national federation's bucket. The caller
// must always pass an explicit override; this test locks that in.
test("federation uses the caller's explicit override, never silently defaults for a non-CBF source", () => {
  assert.equal(newHost.match.tournament.federation, "FGF");
});
...
// Same incident as the tournament-level federation, discovered right after
// fixing it: `home`/`away.federacao` is `null` unless the caller sets
// `clubFederacao` (also always null for real CBF clubs — that part is
// intentional/pre-existing, not something this test is about). Every FGF
// club was missing its federation in the club directory/profile until this.
test("club federacao uses the caller's explicit override", () => {
  assert.equal(newHost.match.home.federacao, "FGF");
  assert.equal(newHost.match.away.federacao, "FGF");
});
