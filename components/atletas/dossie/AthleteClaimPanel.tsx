...
export function AthleteClaimPanel({ bid }: { bid: number }) {
  const { t } = useT();
  ...
  async function reload() {
    ...
    setError("");
    try {
      setContext(await loadAthleteClaimContext(client, { bid, userId: session.userId, role: session.role }));
    } catch {
      setContext(null);
  ...
  useEffect(() => { void reload(); }, [bid]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="mt-2 max-w-md text-left">
      ...
      {canSubmit && expanded && <form className="border border-border bg-background p-4" onSubmit={(event) => { event.preventDefault(); void submit(event.currentTarget); }}><div className="mb-3 flex items-center gap-2"><ShieldCheck size={17} className="text-brand" /><p className="text-sm font-extrabold uppercase">{t("athleteClaim.title")}</p></div><Input id={`athlete-proof-${bid}`} name="documentUrl" type="url" label={t("clubs.claim.document")} placeholder="https://" maxLength={1000} required /><label className="mt-3 flex flex-col gap-1.5"><span className="text-sm font-medium text-muted">{t("clubs.claim.message")}</span><textarea name="message" minLength={20} maxLength={2000} rows={4} required className="border border-border bg-surface px-3.5 py-2.5 text-sm outline-none focus:border-brand" /></label><div className="mt-3 flex flex-wrap gap-2"><Button type="submit" disabled={saving}>{saving ? t("common.loading") : t("clubs.claim.submit")}</Button><Button type="button" variant="secondary" onClick={() => setExpanded(false)} disabled={saving}>{t("common.cancel")}</Button></div>{error && <p role="alert" className="mt-2 text-xs text-danger">{error}</p>}</form>}
