...
export function CorrectionForm({ userId, athletes, corrections, readOnly, onCreated }: { userId: string; athletes: AgentAthleteRecord[]; corrections: CorrectionRequestRecord[]; readOnly: boolean; onCreated: (record: CorrectionRequestRecord) => void }) {
  ...
  const [bid, setBid] = useState(athletes[0] ? String(athletes[0].bid) : "");
  ...
  async function submit(form: HTMLFormElement) {
    ...
    try {
      ...
      const record = await createCorrectionRequest(client, { userId, bid: athlete.bid, field: validated.field, currentValue: currentValue(athlete, validated.field), suggestedValue: validated.suggestedValue, reason: validated.reason, proofUrl: validated.proofUrl });
  ...
  return <div className="mt-4 space-y-5">
    {!readOnly && athletes.length > 0 && <form className="space-y-3 border-b border-border pb-5" onSubmit={(event) => { event.preventDefault(); void submit(event.currentTarget); }}>
      <div><p className="mb-1.5 text-sm font-medium text-muted">{t("agent.correction.athlete")}</p><Select value={bid} onChange={setBid} ariaLabel={t("agent.correction.athlete")} options={athletes.map((athlete) => ({ value: String(athlete.bid), label: `${athlete.name} · ${athlete.bid}` }))} disabled={saving} /></div>
      ...
      <div className="border border-border bg-background px-3 py-2 text-xs"><span className="text-muted">{t("agent.correction.current")}: </span>{currentValue(athletes.find((item) => String(item.bid) === bid), field) ?? "—"}</div>
