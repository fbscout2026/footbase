...
import { updateAgentProfile, type AgentAthleteRecord, type AgentFavoriteAthleteRecord, type AgentFavoriteClubRecord, type AgentFavoriteTournamentRecord, type AgentPanelData, type AgentProfileRecord } from "@/lib/services/agent-panel";
    ...
    <div className="space-y-5">
      ...
      <section className="matchday-surface overflow-hidden">
        ...
        {data.athletes.length === 0 ? <p className="p-8 text-center text-sm text-muted">{t("agent.athletes.empty")}</p> : (
          <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b border-border bg-background text-left text-xs uppercase tracking-wide text-muted"><th className="px-4 py-3">{t("agent.athletes.athlete")}</th><th className="px-4 py-3">ID</th><th className="px-4 py-3">{t("agent.athletes.position")}</th><th className="px-4 py-3">{t("agent.athletes.category")}</th><th className="px-4 py-3">{t("agent.athletes.physical")}</th><th className="px-4 py-3 text-right">{t("common.actions")}</th></tr></thead><tbody>{data.athletes.map((athlete) => <tr key={athlete.fbId} className="border-b border-border/60"><td className="px-4 py-3"><Link href={`/atletas/${athlete.fbId}`} className="font-semibold hover:text-brand">{athlete.name}</Link>{athlete.apelido && <p className="text-xs text-muted">{athlete.apelido}</p>}</td><td className="px-4 py-3 metric-value">{formatAthleteCode(athlete.fbId)}</td><td className="px-4 py-3">{athlete.mainPosition}{athlete.secondaryPosition ? ` / ${athlete.secondaryPosition}` : ""}</td><td className="px-4 py-3">{athlete.currentCategory ?? "—"}</td><td className="px-4 py-3">{athlete.heightCm ?? "—"} cm · {athlete.weightKg ?? "—"} kg</td><td className="px-4 py-3 text-right">{readOnly ? <Link href={`/atletas/${athlete.fbId}`} className="font-bold uppercase text-brand">{t("common.view")}</Link> : <button type="button" onClick={() => setEditingAthlete(athlete)} className="inline-flex min-h-11 items-center gap-2 border border-border px-3 font-bold uppercase hover:border-brand"><Pencil size={15} />{t("common.edit")}</button>}</td></tr>)}</tbody></table></div>
        )}
      </section>

      <AgentFavoriteAthletes favorites={data.favoriteAthletes} />
...
function AgentFavoriteAthletes({ favorites }: { favorites: AgentFavoriteAthleteRecord[] }) {
  const { t } = useT();
  return (
    <section className="matchday-surface overflow-hidden">
      ...
      {favorites.length === 0 ? <p className="p-8 text-center text-sm text-muted">{t("clubPanel.favorites.empty")}</p> : (
